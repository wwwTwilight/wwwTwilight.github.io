...
win（文件名可能不是clash-core.exe，但是应该很明显）
```
resources\static\files\win\x64\clash-core.exe
```

接下来将原本的内核改一个名字，以防万一，然后不管原来那个内核，将新的内核放到那个目录下，改名字为clash-core.exe（根据你旧内核的名字改，旧内核是什么名字就改成什么名字），然后打开clash，按照网上的帖子说，理论上应该就可以正常使用了。
...
# 内核绑定
这个原因是clash应用找不到内核，接下来是解决方式，首先是windows的，先退出clash，然后打开自带的文件资源管理器，在地址栏输入 ```%USERPROFILE%/.config/mihomo``` 进入文件夹
